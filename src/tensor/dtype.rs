...
impl TensorOpError {
    pub fn into_tc_error(self) -> TCError {
...
pub struct TensorDtypeGuard;

impl TensorDtypeGuard {
    pub fn validate(tensor: &Tensor) -> Result<(), TensorOpError> {
    ...
    pub fn validate_pair(left: &Tensor, right: &Tensor) -> Result<(), TensorOpError> {
        Self::validate(left)?;
        Self::validate(right)?;
...
pub fn tensor_op_result<T>(result: Result<T, TensorOpError>) -> TCResult<T> {
