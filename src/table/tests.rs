...
fn simple_schema() -> TableSchema {
    ...
    TableSchema::new(key, values, Vec::new(), BTreeStorageConfig::default())
...
fn schema_with_index() -> TableSchema {
    ...
    TableSchema::new(key, values, indices, BTreeStorageConfig::default())
...
fn rollback_discards_pending_and_unblocks() {
    run_async_test("rollback_discards_pending_and_unblocks", || {
        Box::pin(async {
            ...
            table.rollback(tx(10)).expect("rollback 10");
...
fn duplicate_commit_is_idempotent_table() {
    run_async_test("duplicate_commit_is_idempotent_table", || {
        Box::pin(async {
            ...
            table.commit(tx(10)).expect("first commit");
            table.commit(tx(10)).expect("second commit should be no-op");
            table.finalize(tx(10)).await.expect("finalize 10");
...
fn empty_table_semantics_across_lifecycle_table() {
    run_async_test("empty_table_semantics_across_lifecycle_table", || {
        Box::pin(async {
            ...
            table.commit(tx(95)).expect("empty commit should be allowed");
            table
                .finalize(tx(95))
                .await
...
fn repeated_rollback_and_finalize_are_idempotent_table() {
    run_async_test("repeated_rollback_and_finalize_are_idempotent_table", || {
        Box::pin(async {
            ...
            table.rollback(tx(92)).expect("first rollback");
            table
                .rollback(tx(92))
            ...
            table.commit(tx(93)).expect("commit 93");

            table.finalize(tx(93)).await.expect("first finalize");
            table
                .finalize(tx(93))
                .await
